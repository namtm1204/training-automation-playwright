...
import testCaseData from "../../test-data/change-color/TC-02.json";
...
test.describe("Change color", () => {
  ...
  test(`[TC-02] Verify color is changed successfully when Publish random color for Secondary Color`, async () => {
    ...
    await test.step("Step 2: Change secondary color ", async () => {
      await corporateBrandingPage.changeSecondaryColor(
        testCaseData.secondaryColor
      );
    });
    ...
    await test.step("VP: Verify secondary color of Corporate Branding page", async () => {
      await corporateBrandingPage.verifySecondaryColorOfCorporateBrandingPage(
        rgbColor
      );
    });
    await test.step("VP: Verify secondary color of Locations page", async () => {
      await corporateBrandingPage.verifySecondaryColorLocationPage(
        locationsPage,
        rgbColor
      );
    });
    await test.step("VP: Verify secondary color of Employee List page", async () => {
      await corporateBrandingPage.verifySecondaryColorEmployeeListPage(
        employeeListPage,
        rgbColor
      );
    });
